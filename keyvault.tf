...
resource "azurerm_key_vault_access_policy" "midas" {
    key_vault_id = azurerm_key_vault.common.id

    tenant_id = data.azurerm_client_config.current.tenant_id
    object_id = azurerm_user_assigned_identity.midas.principal_id
...
resource "azurerm_key_vault_access_policy" "azuregcpvaultsync" {
    key_vault_id = azurerm_key_vault.common.id

    tenant_id = data.azurerm_client_config.current.tenant_id
    object_id = azurerm_user_assigned_identity.azuregcpvaultsync.principal_id
